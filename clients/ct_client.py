...
from fl_experiment_config import (
    USE_GIN, CHECKPOINT_DIR, CT_METRICS_FILE, SERVER_PORT,
    WANDB_PROJECT, MODEL_CONFIG, TRAIN_CONFIG, TRAIN_FOLDER, VAL_CT_FOLDER
)
...
class CTClient(fl.client.NumPyClient):
    def __init__(self):
        ...
        self.train_dataset = OrganMaskDataset(TRAIN_FOLDER, modality="CT")
        self.val_dataset = OrganMaskDataset(VAL_CT_FOLDER, modality="CT")
    ...
    def fit(self, parameters, config):
        if self.current_round == 0:
            ...
            print(f"[CT Client] Training: GIN is {gin_status}")
        ...
        print(f"[CT Client] Round {self.current_round} training complete. Train Loss: {epoch_train_loss:.4f}, Train Dice: {epoch_train_dice:.4f}, Val Dice: {val_dice:.4f}")
...
if __name__ == "__main__":
    fl.client.start_numpy_client(server_address=f"localhost:{SERVER_PORT}", client=CTClient())
